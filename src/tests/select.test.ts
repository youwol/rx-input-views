...
test('select', (done) => {
    ...
    let items$ = new BehaviorSubject([new Select.ItemData("a","a"), new Select.ItemData("b","b")])
    ...
    items$.next([new Select.ItemData("a","a"), new Select.ItemData("b","b"), new Select.ItemData("c","c")])
    id$.next('c')
