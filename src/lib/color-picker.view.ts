...
export namespace ColorPicker {
    ...
    export class State {
        ...
        constructor(color$: BehaviorSubject<string> | string = "#ffbb00") {
            this.color$ = (color$ instanceof BehaviorSubject)
                ? color$
                : new BehaviorSubject<string>(color$ as string)
        }
    }
    ...
    export class View implements VirtualDOM {
        ...
        public readonly onchange = (event:any) => this.state.color$.next(event.target.value)
